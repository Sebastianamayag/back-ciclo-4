...
app.get("/libros/buscar/:id", async (req, res) => {
    ...
    if (booksid) {
        const libros = await Libros.findOne({ where: { id: booksid }, include: [{ model: Authors }, { model: Categorias }, { model: Editorial }], attributes: { exclude: ['AuthorId', 'CategoryId', 'EditorialId'] } })
        if (libros) {
            res.status(200).json({ products: libros });
        }
        else {
            res.status(400).json({ "error": `No se ha encontrado el libro con el id: ${booksid}` })
        }
    } else {
